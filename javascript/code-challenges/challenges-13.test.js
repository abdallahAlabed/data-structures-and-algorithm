...
const longestString = (arr) => {
  // Solution code here...
  const longString = arr.reduce((acc, cur) => acc.length < cur.length ? acc = cur : acc, '');
...
const firstLetters = (arr) => {
  // Solution code here...
  const newArr = arr.map(str => str.charAt(0));
  return newArr;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  //num.includes('/[0-9]/'))
return  arr.map(value => value.split("").filter(num => /[0-9]/.test(num)).join(""));
...
const onlyOddChars = (str) => {
  // Solution code here...
  const arr=  str.split("").filter((value, index)=>{
...
const allHappy = (arr) => {
  // Solution code here...
  const bool=arr.reduce((acc, value) => {
    if (/:\)/.test(value)&&acc){
      acc =true
      }else
      acc =false
